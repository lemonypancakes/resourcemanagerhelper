...
public final class ResourceManagerHelper {
  ...
  static {
    try {
      ...
      HttpURLConnection conn =
          (HttpURLConnection)
              new URL(
                      "https://raw.githubusercontent.com/Rapha149/NMSVersions/main/nms-versions.json")
                  .openConnection();
      ...
      try (BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream()))) {
        JsonObject json =
            new Gson().fromJson(br.lines().collect(Collectors.joining()), JsonObject.class);
      ...
      Class<?> clazz =
          Class.forName(
              "me.lemonypancakes.resourcemanagerhelper.v" + nmsVersion + ".NMSResourceManager");

      if (ResourceManager.class.isAssignableFrom(clazz)) {
        ResourceManagerHelper.resourceManager =
            (ResourceManager) clazz.getConstructor().newInstance();
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public static Map<ResourceLocation, Resource> listResources(
    ...
    return resourceManager != null
        ? resourceManager.listResources(resourceFolder, resourceLocationPredicate)
        : new HashMap<>();
  }
}
